...
export default function Consent() {
    ...
    return (
        <div className="container prose dark:prose-invert">
            <div className="mb-5">
                <div className="text-center">
                    {data?.interaction?.client?.logoUri ? (
                        <img className="mr-auto ml-auto mt-0 pt-0 mb-2 pb-0" src={data?.interaction?.client?.logoUri} width="100" />
                    ) : (
                        ""
                    )}
                    <span className="font-medium">{data?.interaction?.client?.clientName}</span> would like to access your:
                </div>
                <ul>
            ...
            <div>
                <form autoComplete="false" action={"/interaction/" + data?.interaction?.uid + "/confirm"} method="post">
                    <button
                        type="submit"
