  ...
  class Session extends hasFormat(provider, "Session", instance(provider).BaseModel) {
    ...
    static async get(ctx) {
      const cookies = ctx.oidc
        ? ctx.oidc.cookies
        : provider.app.createContext(ctx.req, ctx.res).cookies;
      cookies.secure = !cookies.secure && ctx.secure ? true : cookies.secure;

      // I think here is where we can inject multi account support.
      // I.e. get current session cookie id here, also need to support that in session setter.
      // What happens if there's existing interaction/etc. cookies? I'm guessing they'll be invalidated if
      // they're for a different session?
    ...
    loginAccount(details) {
      const { transient = false, accountId, loginTs = epochTime(), amr, acr } = details;
